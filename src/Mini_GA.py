...
def find_mw(smiles_fragment):
    mw_counter = 0
    #counter for implicit hydrogens
    num_H = 0


    for num, character in enumerate(smiles_fragment):
        if character.isupper():
            #add element's MW
            temp_element = pt.elements.isotope(character)
            mw_counter += temp_element.mass

            #find element's implicit hydrogens
            #if num ==
...
def main():
    ...
    read_file = open('../input_files/smilesText.txt','r')
    ...
    for monomer in smiles_list:
        mw_list.append(find_mw(monomer))

    #Create inital population of 10 polymers with
